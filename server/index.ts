...
const isProduction = process.env.NODE_ENV?.toLowerCase().startsWith('prod') === true

const pool = new Pool({ connectionString: process.env.DATABASE_URL, max: 5, ssl: { rejectUnauthorized: false } })
const cookieOptions = { httpOnly: true, sameSite: 'lax' as const, secure: isProduction, maxAge: 1000 * 60 * 60 * 24 * 7 }
const googleStateCookieOptions = { httpOnly: true, sameSite: 'lax' as const, secure: isProduction, maxAge: 10 * 60 * 1000 }
